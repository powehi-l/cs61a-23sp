    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if len(player.hand) != 0:
            ...
            for i in range(len(player.hand)):
                if player.hand[i].power(opponent_card) > max:
                    index, max = i, player.hand[i].power(opponent_card)
            best_card = player.hand.pop(index)
            self.attack, self.defense = self.attack + best_card.attack, self.defense + best_card.defense
        # You should add your implementation above this.
        if best_card:
