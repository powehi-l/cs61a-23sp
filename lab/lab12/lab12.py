...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        if OPERATORS.get(exp.first):
            operator = OPERATORS[exp.first] # UPDATE THIS FOR Q3
        else:
            operator = bindings[exp.first]
        ...
        return exp
    elif bindings[exp]:  # CHANGE THIS CONDITION FOR Q5
...
def floor_div(expr):
    ...
    """
    # BEGIN SOLUTION Q3
    cur, value = expr, expr.first
    while not cur.rest is nil:
        # print(expr)
        cur, value = cur.rest, value // cur.rest.first
    return value
...
def eval_and(operands):
    ...
    if operands is nil:
        return True
    cur, value = operands, operands.first
    if isinstance(value, Pair):
        res = calc_eval(value)
    else:
        res = value

    if res is not 0 and not res:
        return False
    
    if cur.rest == nil:
        return res
    else:
        return eval_and(cur.rest)
