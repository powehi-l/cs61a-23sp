    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):
                ...
                self.place, place.ant = place, self
                self.ant_contained.place = self.place
                # may need modify the original ant's place to None
            else: 
...
# 对于container里面的Ant，他的place参数是没有指定的，那怎么找他的下一个place呢？
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 1
        "*** YOUR CODE HERE ***"
        # why this is not right, I don't get it.
        # target.slow_turn = 5
        # def new_bee_action(gamestate):
        #     if gamestate.time % 2 == 0:
        #         Bee.action(target, gamestate)
        #     target.slow_turn -= 1
        #     if target.slow_turn == 0:
        #         target.action = Bee.action
        # target.action = new_bee_action
        target.slow_turn = 5
        def slow_bee_action(gamesate):
            if target.slow_turn:
                if gamesate.time % 2 == 0:
                    Bee.action(target, gamesate)
                target.slow_turn -= 1
            else:
                Bee.action(target, gamesate)
...
class LaserAnt(ThrowerAnt):
        ...
        while not place.is_hive:
            if place.ant:
                ...
                if place.ant.is_container and place.ant.ant_contained and place.ant.ant_contained != self:
                    res[place.ant.ant_contained] = distance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(0, self.damage - 0.25 * distance)
        # END Problem Optional 2
