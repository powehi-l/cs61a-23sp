...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    tens, ones = opponent_score % 100 // 10, player_score % 10
    assert type(tens) == int, type(ones) == int
    return max(3*abs(tens-ones),1)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    "*** YOUR CODE HERE ***"
    last = strategy(0, 0)
    for first_score in range(goal):
        for second_score in range(goal):
            if strategy(first_score, second_score) != last:
                return False
            last = strategy(first_score, second_score)
    return True
    # END PROBLEM 7
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    if boar_strategy(score, opponent_score, threshold, num_rolls) == 0:
        return 0
