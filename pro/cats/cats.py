...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    typed_size, source_size = len(typed_words), len(source_words)
    ...
    i, dismatch = 0, 0
    while i != typed_size and i != source_size:
        if typed_words[i] != source_words[i]:
            dismatch += 1
        i = i + 1
    
    if i == typed_size:
        return (typed_size - dismatch) / typed_size * 100.0
    else:
        return (i - dismatch) / typed_size * 100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    smallest_difference, smallest_word = 100.0, typed_word

    for word in word_list[::-1]:
        difference = diff_function(typed_word, word, limit)
        if difference <= smallest_difference:
            smallest_difference, smallest_word = difference, word
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if limit < 0 or len(typed) == 0 or len(source) == 0: # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
        return max(len(typed), len(source))
        # END
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    typed_size, source_size = len(typed), len(source)
    i, dismatch = 0, 0
    while i != typed_size and i != source_size:
        if typed[i] != source[i]:
            break
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    times = []
    for player in range(len(timestamps_per_player)):
        oneplayer = []
        for word in range(len(words)):
            oneplayer.append(timestamps_per_player[player][word + 1] - timestamps_per_player[player][word])
...
def fastest_words(match):
    ...
    for i in player_indices:
        final.append([])
    # print(final)
    # print(player_indices, word_indices)
    for word in word_indices:
        min = len(get_all_times(match)) - 1
        for player in player_indices[::-1]:
            if time(match, player, word) <= time(match, min, word):
                min = player
        final[min].append(get_word(match, word))
    return final
